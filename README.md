...
// Transform — runtime method interception
ForgeVM.transformer().load(new MyTransformer());
...
| `forgevm.transform` | Runtime bytecode rewriting — method interception |
...
// Load / unload
ForgeVM.transformer().load(new ProcessLogger());
ForgeVM.transformer().unload(ProcessLogger.class);
```
...
| `forgevm.transform` | 运行时字节码重写 — 方法拦截 |
